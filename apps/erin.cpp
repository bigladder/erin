...
main(int argc, char** argv)
{
    ...
    {
        std::string tomlFilename;
        auto pack = app.add_subcommand("pack", "Pack loads into a single csv file");
        pack->add_option("toml_file", tomlFilename, "TOML filename")->required();

        std::string loadsFilename = "loads.csv";
        pack->add_option(
                "-o,--out", loadsFilename, "Packed-loads csv filename; default:loads.csv"
        );
       ...
       pack->add_flag("-v,--verbose", verbose, "Verbose output");
        ...
        pack->callback(
                [&]() {
                    result = packCommand(
                            tomlFilename, loadsFilename, verbose
                    );
                }
        );
    }
