...
class ReliabilityCoordinator
{
  public:
    ...
    size_t add_failure_mode(std::string const& tag,
                            size_t const& failure_dist_id,
                            size_t const& repair_dist_id);

    size_t link_component_with_failure_mode(size_t const& comp_id, size_t const& fm_id);

    std::vector<TimeState> make_schedule_for_link(size_t linkId,
  ...
  private:
    ...
    double calc_next_event(size_t link_id,
                           double dt_fm,
                           const std::function<double()>& rand_fn,
                           const DistributionSystem& cds,
                           bool is_failure) const;

    bool update_single_schedule(double& time,
                                double& dt,
...
bool schedule_state_at_time(const std::vector<TimeState>& schedule,
                            double time,
