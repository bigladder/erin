...
void ensure_port_not_already_added(std::set<size_t> const& ports, size_t port_number)
{
...
void record_port_number(ComponentAndPort const& c, std::map<std::string, PortCounts>& ports)
{
    ...
    if (!ports.contains(id))
    {
        ...
        {
            ports.emplace(id, PortCounts {std::set<size_t> {c.port_number}, std::set<size_t> {}});
        }
        else if (c.port_type == PortType::Outflow)
        {
            ports.emplace(id, PortCounts {std::set<size_t> {}, std::set<size_t> {c.port_number}});
        }
        else
        {
...
std::string build_label(std::string const& id, PortCounts const& pc)
{
...
std::string build_label_html(std::string const& id, PortCounts const& pc)
{
    std::ostringstream label;
    label << "<\n"
...
std::string network_to_dot(std::vector<Connection> const& network,
