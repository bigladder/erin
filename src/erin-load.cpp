...
std::vector<std::optional<Load>> ParseMultiLoadFileLoad(toml::table const& table,
                                                        std::string const& tag)
{
    std::string tableFullName = "loads." + tag;
...
std::optional<Load> ParseSingleLoad(std::string const& tag,
...
std::vector<std::optional<Load>> ParseMultiLoad(std::string const& tableName,
                                                toml::table const& table)
{
...
bool MaybePushLoad(std::optional<Load> maybeLoad,
                ...
                for (auto it2 = table2.cbegin(); it2 != table2.cend(); ++it2)
                {
                    ...
                    if (key == "csv_file")
                    {
                        maybeLoad = ParseSingleLoad(
                            tag, table2, tableName, explicitValidation, fileValidation);
                    ...
                    if (key == "multi_part_csv")
                    {
                        ...
                        for (auto& anotherMaybeLoad : maybeLoads)
                        {
                            ...
                            {
                                MaybePushLoad(anotherMaybeLoad, tableName, loads);
                            }
                            else
                            {
...
int write_packed_loads(const std::vector<Load>& loads, std::string const& loadsFilename)
{
    ...
    if (!out.good())
    {
        std::cout << "Could not open '" << loadsFilename << "' for writing." << std::endl;
        return EXIT_FAILURE;
    }
