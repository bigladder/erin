...
void ActivateConnectionsForReliability(Model& m, SimulationState& ss, double time)
{
                                                  ...
                                                  size_t compIdx)
{
    ...
    flow_t outflowAvailable = ss.Flows[cec.OutflowConn].Available_W > cec.MaxOutflow_W
                                  ? cec.MaxOutflow_W
    ...
    if (lossflowConn.has_value())
    {
        ...
        flow_t nonOutflowAvailableLimited =
            nonOutflowAvailable > cec.MaxLossflow_W ? cec.MaxLossflow_W : nonOutflowAvailable;
                                            ...
                                            size_t compIdx)
{
    ...
    flow_t outflowRequest = ss.Flows[outflowConnIdx].Requested_W > cec.MaxOutflow_W
                                ? cec.MaxOutflow_W
                                : ss.Flows[outflowConnIdx].Requested_W;
    flow_t inflowRequest = static_cast<flow_t>(std::ceil(outflowRequest / cec.Efficiency));
...
void UpdateEnvironmentFlowForMover(Model const& m, SimulationState& ss, size_t moverIdx)
{
    ...
    flow_t outflowAvail = ss.Flows[mov.OutflowConn].Available_W > mov.MaxOutflow_W
                              ? mov.MaxOutflow_W
                                     ...
                                     flow_t remainingRequest_W)
{
    for (size_t inflowConn : inflowConns)
    {
        ...
        remainingRequest_W = remainingRequest_W > ss.Flows[inflowConn].Available_W
...
void RunMuxBackward(Model& model, SimulationState& ss, size_t muxIdx)
{
    ...
    Mux_RequestInflowsIntelligently(ss, model.Muxes[muxIdx].InflowConns, totalRequest);
}
                                           ...
                                           size_t compIdx)
{
    ...
    flow_t outflowAvailable = static_cast<flow_t>(std::floor(cec.Efficiency * inflowAvailable));
    if (outflowAvailable > cec.MaxOutflow_W)
    {
        outflowAvailable = cec.MaxOutflow_W;
    }
    assert((inflowAvailable >= outflowAvailable) &&
...
void RunMuxForward(Model& model, SimulationState& ss, size_t muxIdx)
{
    ...
    {
        totalAvailable = UtilSafeAdd(totalAvailable, ss.Flows[inflowConnIdx].Available_W);
    }
    ...
    if (totalAvailable > 0)
    {
        for (size_t i = 0; i < mux.NumOutports; ++i)
        {
            if (mux.MaxOutflows_W[i] > outflowAvailables[i])
            {
                ...
                flow_t toAdd = maxAdd > totalAvailable ? totalAvailable : maxAdd;
...
void RunStoreForward(Model& model, SimulationState& ss, size_t inflowConnIdx, size_t storeIdx)
{
    ...
    flow_t dischargeAvailable = ss.StorageAmounts_J[storeIdx] > 0 ? store.MaxDischargeRate_W : 0;
...
void RunStorePostFinalization(
    Model& model, SimulationState& ss, double t, size_t connIdx, size_t compIdx)
{
    ...
    std::optional<size_t> maybeOutflowConn =
        FindOutflowConnection(model, model.Connections[connIdx].To, compIdx, 0);
    assert(maybeOutflowConn.has_value() && "store must have an outflow connection");
    ...
    else if (netCharge_W < 0 && (ss.StorageAmounts_J[compIdx] > store.ChargeAmount_J))
    {
        if (store.WasteflowConn.has_value())
        {
            size_t wfIdx = store.WasteflowConn.value();
...
void UpdateStoresPerElapsedTime(Model const& m, SimulationState& ss, double elapsedTime)
{
    for (size_t storeIdx = 0; storeIdx < m.Stores.size(); ++storeIdx)
    {
        ...
        netEnergyAdded += std::lround(elapsedTime * static_cast<double>(ss.Flows[inConn].Actual_W));
        netEnergyAdded -=
            std::lround(elapsedTime * static_cast<double>(ss.Flows[outConn].Actual_W));
        if (store.WasteflowConn.has_value())
        {
            size_t wConn = store.WasteflowConn.value();
            netEnergyAdded -=
                std::lround(elapsedTime * static_cast<double>(ss.Flows[wConn].Actual_W));
        }
        assert(static_cast<long>(m.Stores[storeIdx].Capacity_J - ss.StorageAmounts_J[storeIdx]) >=
                   netEnergyAdded &&
               "netEnergyAdded cannot put storage over capacity");
        if (netEnergyAdded < -1 * static_cast<long>(ss.StorageAmounts_J[storeIdx]))
        {
            ...
            int64_t inflow = static_cast<int64_t>(ss.Flows[inConn].Actual_W);
        ...
        assert(netEnergyAdded >= (-1 * static_cast<long>(ss.StorageAmounts_J[storeIdx])) &&
...
std::string ToString(ComponentType compType)
{
    ...
    default:
    {
        WriteErrorMessage("ToString", "unhandled component type: " + ToString(compType));
        std::exit(1);
    }
    }
    return result;
...
{
    std::cout << "time: " << time_s << " s, " << TimeToISO8601Period(static_cast<uint64_t>(time_s))
              << ", " << TimeInSecondsToHours(static_cast<uint64_t>(time_s)) << " h" << std::endl;
    for (size_t flowIdx = 0; flowIdx < ss.Flows.size(); ++flowIdx)
    {
        std::cout << ConnectionToString(m.ComponentMap, m.Connections[flowIdx]) << ": "
                  << ss.Flows[flowIdx].Actual_W << " (R: " << ss.Flows[flowIdx].Requested_W
                  << "; A: " << ss.Flows[flowIdx].Available_W << ")" << std::endl;
    }
}
...
{
    int netDischarge = static_cast<int>(s.StorageDischarge) - static_cast<int>(s.StorageCharge);
    int sum = static_cast<int>(s.Inflow) + netDischarge + static_cast<int>(s.EnvInflow) -
              (static_cast<int>(s.OutflowAchieved) + static_cast<int>(s.Wasteflow));
    double eff = (static_cast<double>(s.Inflow) + static_cast<double>(netDischarge)) > 0.0
    ...
    std::cout << "  Efficiency             : " << eff << "%" << " (= " << s.OutflowAchieved << "/"
...
ComponentIdAndWasteAndEnvironmentConnection Model_AddMover(
    Model& m, double cop, size_t inflowTypeId, size_t outflowTypeId, std::string const& tag)
{
    assert(cop > 0.0);
    Mover mov = {
        .COP = cop,
...
std::vector<TimeAndFlows> Simulate(Model& model, bool print = true)
{
    double t = 0.0;
...
size_t
Model_AddConstantSource(Model& m, uint32_t available, size_t outflowTypeId, std::string const& tag)
{
...
Connection Model_AddConnection(
    Model& m, size_t fromId, size_t fromPort, size_t toId, size_t toPort, size_t flowId)
{
...
std::optional<uint32_t> ModelResults_GetStoreState(Model const& m,
                                                   size_t compId,
                                                   double time,
                                                   std::vector<TimeAndFlows> timeAndFlows)
{
                                              ...
                                              std::vector<TimeAndFlows> const& timeAndFlows)
{
    ...
    for (size_t eventIdx = 1; eventIdx < timeAndFlows.size(); ++eventIdx)
    {
        ...
        for (size_t connId = 0; connId < timeAndFlows[eventIdx - 1].Flows.size(); ++connId)
        {
            auto fromType = m.ComponentMap.CompType[m.Connections[connId].FromId];
            auto toType = m.ComponentMap.CompType[m.Connections[connId].ToId];
            ...
            case ComponentType::ScheduleBasedLoadType:
            {
                ...
                sos.OutflowRequest_kJ += (flow.Requested_W / W_per_kW) * dt;
                allLoadsMet = allLoadsMet && (flow.Actual_W == flow.Requested_W);
                if (flow.Actual_W != flow.Requested_W)
                {
                    sos.LoadNotServed_kJ += ((flow.Requested_W - flow.Actual_W) / W_per_kW) * dt;
                }
            }
            break;
            case ComponentType::WasteSinkType:
             ...
             ++storeIdx)
        {
            double stored_J =
                ...
                static_cast<double>(timeAndFlows[eventIdx - 1].StorageAmounts_J[storeIdx]);
            if (stored_J > 0.0)
            {
...
std::optional<TagAndPort> ParseTagAndPort(std::string const& s, std::string const& tableName)
{
    ...
    {
        std::cout << "[" << tableName << "] " << "unable to parse connection string '" << s << "'"
                  << std::endl;
        return {};
    }
...
Result ParseNetwork(FlowDict const& fd, Model& m, toml::table const& table)
{
    ...
    for (size_t i = 0; i < connArray.size(); ++i)
    {
        ...
        {
            std::cout << "[network] " << "unable to parse connection string at [" << i << "][0]"
                      << std::endl;
            return Result::Failure;
        }
        ...
        {
            std::cout << "[network] " << "unable to parse connection string at [" << i << "][1]"
                      << std::endl;
            return Result::Failure;
        }
        ...
        {
            std::cout << "[network] " << "could not find component id for tag '" << from << "'"
                      << std::endl;
            return Result::Failure;
        }
        ...
        {
            std::cout << "[network] " << "could not find component id for tag '" << to << "'"
                      << std::endl;
            return Result::Failure;
        }
...
{
    os << "Flow{Requested_W=" << flow.Requested_W << ", " << "Available_W=" << flow.Available_W
