                            ...
                            ComponentValidationMap const& compValids)
{
    ...
    {
        WriteErrorMessage(fullTableName, "required field 'type' not present");
    ...
    case ComponentType::ConstantSourceType:
    {
        ...
        if (table.contains("max_outflow"))
        {
            ...
            if (!maybe.has_value())
            {
                WriteErrorMessage(fullTableName, "unable to parse 'max_outflow' as number");
                return Result::Failure;
            }
            double maxAvailableReal = maybe.value();
            maxAvailable = static_cast<uint32_t>(Power_ToWatt(maxAvailableReal, rateUnit));
        }
    ...
    case ComponentType::ScheduleBasedLoadType:
    {
        ...
        {
            WriteErrorMessage(fullTableName, "missing required field 'loads_by_scenario'");
            return Result::Failure;
        }
        if (!table.at("loads_by_scenario").is_table())
        {
            WriteErrorMessage(fullTableName, "'loads_by_scenario' must be a table");
            return Result::Failure;
        }
    ...
    case ComponentType::ConstantEfficiencyConverterType:
    {
        ...
        if (efficiency <= 0.0)
        {
            WriteErrorMessage(fullTableName, "efficiency must be > 0.0");
            return Result::Failure;
        }
        ...
        auto const compIdAndWasteConn = Model_AddConstantEfficiencyConverter(
            s.TheModel, efficiency, inflowId, outflowId, lossflowId, tag);
        id = compIdAndWasteConn.Id;
        if (input.contains("rate_unit"))
        {
            ...
            if (!maybeRateUnit.has_value())
            {
                WriteErrorMessage(fullTableName, "unhandled rate unit '" + localRateUnitStr + "'");
                return Result::Failure;
            }
    ...
    case ComponentType::StoreType:
    {
        ...
        uint32_t noChargeAmount_J = static_cast<uint32_t>(chargeAtSoc * capacity_J);
        ...
        {
            auto compIdAndWasteConn = Model_AddStoreWithWasteflow(s.TheModel,
                                                                  capacity_J,
                                                                  maxCharge_W,
                                                                  maxDischarge_W,
                                                                  noChargeAmount_J,
    ...
    case ComponentType::MoverType:
    {
        ...
        auto compIdAndConns = Model_AddMover(s.TheModel, cop, inflowId, outflowId, tag);
...
Result
ParseComponents(Simulation& s, toml::table const& table, ComponentValidationMap const& compValids)
{
    for (auto it = table.cbegin(); it != table.cend(); ++it)
    {
        auto result = ParseSingleComponent(s, it->second.as_table(), it->first, compValids);
