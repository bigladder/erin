...
double weibull_quantile(const double& p, const double& k, const double& a, const double& b)
{
...
size_t DistributionSystem::add_fixed(const std::string& tag, double value_in_seconds)
{
...
size_t
DistributionSystem::add_uniform(const std::string& tag, double lower_bound_s, double upper_bound_s)
{
...
size_t DistributionSystem::add_normal(const std::string& tag, double mean_s, double stddev_s)
{
    auto id {dist.tag.size()};
...
void ensure_sizes_equal(const std::string& tag, const size_t& a, const size_t& b)
{
    if (a != b)
    {
        std::ostringstream oss {};
...
void ensure_size_greater_than_or_equal_to(const std::string& tag, const size_t& a, const size_t& n)
{
    if (a < n)
    {
        std::ostringstream oss {};
...
void ensure_always_increasing(const std::string& tag, const std::vector<double>& xs)
{
    bool first {true};
    double last {0.0};
...
void ensure_for_all(const std::string& tag,
...
void ensure_equals(const std::string& tag, const double x, const double val)
{
    if (x != val)
    {
        std::ostringstream oss {};
...
void ensure_greater_than_or_equal_to(const double x, const double val)
{
    if (x < val)
    {
        std::ostringstream oss {};
...
void ensure_greater_than(const double x, const double val)
{
    if (x <= val)
    {
        std::ostringstream oss {};
...
void ensure_greater_than_zero(const double x) { ensure_greater_than(x, 0.0); }

size_t DistributionSystem::add_quantile_table(const std::string& tag,
                                              const std::vector<double>& xs,
                                              const std::vector<double>& dtimes_s)
{
    ...
    ensure_equals(tag + "[" + std::to_string(last_idx) + "]", xs[last_idx], 1.0);
    auto id {dist.tag.size()};
    auto subtype_id {quantile_table_dist.start_idx.size()};
    size_t start_idx {subtype_id == 0 ? 0 : (quantile_table_dist.end_idx[subtype_id - 1] + 1)};
...
size_t DistributionSystem::add_weibull(const std::string& tag,
...
size_t DistributionSystem::lookup_dist_by_tag(const std::string& tag) const
{
...
std::optional<Distribution> DistributionSystem::get_dist_by_id(size_t id) const
{
...
double DistributionSystem::next_time_advance(size_t dist_id)
{
    auto fraction = roll(g);
    return next_time_advance(dist_id, fraction);
}

double DistributionSystem::next_time_advance(size_t dist_id, double fraction) const
{
    ...
    case DistType::Uniform:
    {
        ...
        auto delta = ub - lb;
        dt = static_cast<double>(fraction * static_cast<double>(delta) + static_cast<uint32_t>(lb));
    }
    break;
    case DistType::Normal:
    {
        ...
        dt = static_cast<double>(std::round(avg + sd * sqrt2 * erfinv(twice * fraction - 1.0)));
    }
    break;
    case DistType::QuantileTable:
    {
        ...
        if (fraction >= 1.0)
        {
            dt = static_cast<double>(std::round(quantile_table_dist.times[end_idx]));
        }
        else
        {
            for (size_t idx {start_idx}; idx < end_idx; ++idx)
            {
                ...
                if ((fraction >= v0) && (fraction < v1))
                {
                    if (fraction == v0)
                    {
                        dt = static_cast<double>(std::round(quantile_table_dist.times[idx]));
                        break;
                    }
                    else
                    {
                        ...
                        dt = static_cast<double>(std::round(time0 + (df / dv) * dtimes));
                    }
                }
            }
        }
    ...
    case DistType::Weibull:
    {
        ...
        dt = static_cast<double>(std::round(weibull_quantile(fraction, k, a, b)));
    }
    break;
    default:
    {
...
void DistributionSystem::print_distributions() const
{
    for (size_t i = 0; i < dist.dist_type.size(); ++i)
    {
        std::cout << i << ": " << dist_type_to_tag(dist.dist_type[i]) << " -- " << dist.tag[i]
                  << std::endl;
    }
}
                          ...
                          DistributionValidationMap const& dvm)
{
    for (auto it = table.cbegin(); it != table.cend(); ++it)
    {
        ...
        if (it->second.is_table())
        {
            ...
            {
                WriteErrorMessage(fullTableName, "missing required field 'type'");
            ...
            default:
            {
                WriteErrorMessage("distribution", "unhandled distribution type: " + distTypeTag);
                std::exit(1);
            }
            break;
            }
