...
namespace erin
{
  ...
  void
  ensure_port_not_already_added(std::set<size_t> const& ports, size_t port_number)
  {
      ...
      std::map<std::string, PortCounts>& ports)
  {
    ...
    if (!ports.contains(id))
    {
      if (c.port_type == PortType::Inflow)
      {
        ports.emplace(
            id,
            PortCounts{std::set<size_t>{c.port_number}, std::set<size_t>{}});
      }
      else if (c.port_type == PortType::Outflow)
      {
        ports.emplace(
            id,
            PortCounts{std::set<size_t>{}, std::set<size_t>{c.port_number}});
      }
      else
      {
