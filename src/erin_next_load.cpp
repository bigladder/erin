                                    ...
                                    std::string const& tag)
{
    ...
    if (table.contains("time_rate_pairs"))
    {
        auto const& timeUnitStr = std::get<std::string>(table.at("time_unit").Value);
        {
            ...
            if (!maybe.has_value())
            {
                WriteErrorMessage(tableFullName, "Unhandled 'time_unit': " + timeUnitStr);
                return {};
            }
            timeUnit = maybe.value();
        }
        auto const& maybeRateUnitStr = std::get<std::string>(table.at("rate_unit").Value);
        {
            auto maybe = TagToPowerUnit(maybeRateUnitStr);
            if (!maybe.has_value())
            {
                WriteErrorMessage(tableFullName, "Unhandled 'rate_unit': " + maybeRateUnitStr);
                return {};
            }
    ...
    {
        std::string csvFileName = std::get<std::string>(table.at("csv_file").Value);
        ...
        while (inputDataFile.is_open() && inputDataFile.good())
        {
            ...
            TimeAndAmount ta {
                ...
                .Amount_W = static_cast<flow_t>(Power_ToWatt(std::stod(pair[1]), rateUnit)),
            };
            trps.push_back(ta);
        }
        inputDataFile.close();
                        ...
                        std::string const& tag)
{
    ...
    while (inputDataFile.is_open() && inputDataFile.good())
    {
        ...
        ta.Amount_W = static_cast<uint32_t>(Power_ToWatt(std::stod(pair[1]), rateUnit));
        trps.push_back(ta);
    }
    inputDataFile.close();
    timeRatePairs = trps;
