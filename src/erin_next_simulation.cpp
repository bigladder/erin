...
void Simulation_PrintComponents(Simulation const& s)
{
    ...
    {
        std::vector<size_t> const& outflowTypes = m.ComponentMap.OutflowType[i];
        ...
        for (size_t inport = 0; inport < inflowTypes.size(); ++inport)
        {
            ...
            {
                std::cout << "- inport " << inport << ": " << s.FlowTypeMap.Type[inflowType]
                          << std::endl;
            }
        }
        for (size_t outport = 0; outport < outflowTypes.size(); ++outport)
        {
            ...
            {
                std::cout << "- outport " << outport << ": " << s.FlowTypeMap.Type[outflowType]
                          << std::endl;
            }
        }
        ...
        {
            ScheduleBasedLoad const& sbl = m.ScheduledLoads[m.ComponentMap.Idx[i]];
            for (auto const& keyValue : sbl.ScenarioIdToLoadId)
            {
                std::cout << "-- for scenario: " << s.ScenarioMap.Tags[keyValue.first]
                          << ", use load: " << s.LoadMap.Tags[keyValue.second] << std::endl;
            }
        }
        break;
        case ComponentType::ScheduleBasedSourceType:
        {
            ScheduleBasedSource const& sbs = m.ScheduledSrcs[m.ComponentMap.Idx[i]];
            for (auto const& keyValue : sbs.ScenarioIdToSourceId)
            {
                std::cout << "-- for scenario: " << s.ScenarioMap.Tags[keyValue.first]
                          << ", use supply: " << s.LoadMap.Tags[keyValue.second] << std::endl;
            }
        ...
        {
            ConstantEfficiencyConverter const& cec = m.ConstEffConvs[m.ComponentMap.Idx[i]];
                                      ...
                                      ValidationInfo const& validationInfo)
{
    ...
    {
        WriteErrorMessage("simulation_info", "Required section [simulation_info] not found");
        return Result::Failure;
    }
    toml::value const& simInfoValue = v.at("simulation_info");
    if (!simInfoValue.is_table())
    {
        WriteErrorMessage("simulation_info", "Required section [simulation_info] is not a table");
        return Result::Failure;
    }
                             ...
                             ValidationInfo const& fileValidation)
{
    ...
    auto maybeLoads = ParseLoads(loadTable.as_table(), explicitValidation, fileValidation);
                                            ...
                                            toml::table const& fcData)
{
    ...
    {
        std::cout << "[" << tableFullName << "] " << "missing required field 'lower_bound'"
                  << std::endl;
        return Result::Failure;
    }
    if (!(fcData.at("lower_bound").is_floating() || fcData.at("lower_bound").is_integer()))
    {
        std::cout << "[" << tableFullName << "] " << "field 'lower_bound' not a number"
                  << std::endl;
        return Result::Failure;
    }
    ...
    {
        std::cout << "[" << tableFullName << "] " << "field 'lower_bound' has no value"
                  << std::endl;
        return Result::Failure;
    }
    ...
    {
        std::cout << "[" << tableFullName << "] " << "missing required field 'upper_bound'"
                  << std::endl;
        return Result::Failure;
    }
    if (!(fcData.at("upper_bound").is_floating() || fcData.at("upper_bound").is_integer()))
    {
        std::cout << "[" << tableFullName << "] " << "field 'upper_bound' not a number"
                  << std::endl;
        return Result::Failure;
    }
    ...
    {
        std::cout << "[" << tableFullName << "] " << "field 'upper_bound' has no value"
                  << std::endl;
        return Result::Failure;
    }
...
Result Simulation_ParseFragilityCurves(Simulation& s, toml::value const& v)
{
    if (v.contains("fragility_curve"))
    {
        ...
        for (auto const& pair : v.at("fragility_curve").as_table())
        {
            ...
            {
                std::cout << "[" << tableFullName << "] not a table" << std::endl;
                return Result::Failure;
            }
            ...
            if (!fcData.contains("type"))
            {
                std::cout << "[" << tableFullName << "] "
                          << "does not contain required value 'type'" << std::endl;
                return Result::Failure;
            }
            ...
            {
                std::cout << "[" << tableFullName << "] " << "could not interpret type as string"
                          << std::endl;
                return Result::Failure;
            }
...
Result Simulation_ParseFailureModes(Simulation& s, toml::value const& v)
{
    if (v.contains("failure_mode"))
    {
        if (!v.at("failure_mode").is_table())
        {
            WriteErrorMessage("failure_mode", "failure_mode section must be a table");
            return Result::Failure;
        }
        ...
        for (auto const& pair : fmTable)
        {
            ...
            {
                WriteErrorMessage(fullName, "missing required field 'failure_dist'");
                return Result::Failure;
            }
...
Result Simulation_ParseFragilityModes(Simulation& s, toml::value const& v)
{
    if (v.contains("fragility_mode"))
    {
        ...
        for (auto const& pair : fmTable)
        {
            ...
            if (!pair.second.is_table())
            {
                WriteErrorMessage(fullName, "fragility_mode section must be a table");
                return Result::Failure;
            }
            toml::table const& fmValueTable = pair.second.as_table();
            if (!fmValueTable.contains("fragility_curve"))
            {
                WriteErrorMessage(fullName, "missing required field 'fragility_curve'");
                return Result::Failure;
            }
            if (!fmValueTable.at("fragility_curve").is_string())
            {
                WriteErrorMessage(fullName, "'fragility_curve' field must be a string");
                return Result::Failure;
            }
            ...
            if (fmValueTable.contains("repair_dist"))
            {
                ...
                {
                    WriteErrorMessage(fullName, "field 'repair_dist' must be a string");
                    return Result::Failure;
                }
                                     ...
                                     DistributionValidationMap const& dvm)
{
    if (v.contains("dist") && v.at("dist").is_table())
    {
        ...
        return ParseDistributions(s.TheModel.DistSys, v.at("dist").as_table(), dvm);
    }
    std::cout << "required field 'dist' not found" << std::endl;
    return Result::Failure;
}

Result Simulation_ParseNetwork(Simulation& s, toml::value const& v)
{
...
Result Simulation_ParseScenarios(Simulation& s, toml::value const& v)
{
    if (v.contains("scenarios") && v.at("scenarios").is_table())
    {
        ...
        if (result == Result::Success)
        {
            for (auto const& pair : v.at("scenarios").as_table())
            {
                ...
                {
                    std::cout << "[scenarios] " << "could not find scenario id for '"
                ...
                if (!pair.second.is_table())
                {
                    std::cout << "[" << fullName << "] " << "must be a table" << std::endl;
                }
                ...
                if (data.contains("intensity"))
                {
                    ...
                    {
                        std::cout << "[" << fullName << ".intensity] " << "must be a table"
                                  << std::endl;
                        return Result::Failure;
                    }
                    for (auto const& p : data.at("intensity").as_table())
                    {
                        ...
                        {
                            std::cout << "[" << fullName << ".intensity." << intensityTag << "] "
    ...
    std::cout << "required field 'scenarios' not found or not a table" << std::endl;
    return Result::Failure;
}

std::optional<Simulation> Simulation_ReadFromToml(toml::value const& v,
                                                  InputValidationMap const& validationInfo)
{
    Simulation s = {};
    Simulation_Init(s);
    auto simInfoResult = Simulation_ParseSimulationInfo(s, v, validationInfo.SimulationInfo);
    ...
    auto loadsResult = Simulation_ParseLoads(
        s, v, validationInfo.Load_01Explicit, validationInfo.Load_02FileBased);
    ...
    auto distResult = Simulation_ParseDistributions(s, v, validationInfo.Dist);
                          ...
                          std::vector<size_t> const& compOrder)
{
    ...
    out << "scenario id," << "scenario start time (P[YYYY]-[MM]-[DD]T[hh]:[mm]:[ss]),"
        << "elapsed (hours)";
    for (std::string const& prefix : std::vector<std::string> {"", "REQUEST:", "AVAILABLE:"})
    {
        for (auto const& connId : connOrder)
        {
            ...
            out << "," << prefix << ConnectionToString(compMap, conn, true) << " (kW)";
        }
    }
         ...
         std::vector<std::pair<std::string, std::string>> {{"Stored: ", " (kJ)"}, {"SOC: ", ""}})
    {
        for (size_t storeIdx : storeOrder)
        {
            for (size_t compId = 0; compId < compMap.Tag.size(); ++compId)
            {
                ...
                {
                    out << "," << prePostFix.first << compMap.Tag[compId] << prePostFix.second;
                }
            }
        }
    }
...
std::string DoubleToString(double value, unsigned int precision)
{
    ...
    return std::to_string(static_cast<uint32_t>(std::trunc(value))) +
           (fractionPart > 0.0 ? ("." + std::to_string(static_cast<uint32_t>(fractionPart))) : "");
}
                             ...
                             std::vector<size_t> const& compOrder)
{
    ...
    for (auto const& r : results)
    {
        ...
        out << scenarioTag << "," << scenarioStartTimeTag << "," << (r.Time / seconds_per_hour);
        for (size_t const& i : connOrder)
        {
            double actual_kW = static_cast<double>(r.Flows[i].Actual_W) / W_per_kW;
            out << "," << DoubleToString(actual_kW, 1);
        }
        for (size_t const& i : connOrder)
        {
            double req_kW = static_cast<double>(r.Flows[i].Requested_W) / W_per_kW;
            out << "," << DoubleToString(req_kW, 1);
        }
        for (size_t const& i : connOrder)
        {
            double avail_kW = static_cast<double>(r.Flows[i].Available_W) / W_per_kW;
        ...
        {
            double store_kJ = static_cast<double>(r.StorageAmounts_J[i]) / J_per_kJ;
...
SetLoadsForScenario(std::vector<ScheduleBasedLoad>& loads, LoadDict loadMap, size_t scenarioIdx)
{
    for (size_t sblIdx = 0; sblIdx < loads.size(); ++sblIdx)
    {
        ...
        {
            std::cout << "ERROR:" << "Unhandled scenario id in ScenarioIdToLoadId" << std::endl;
            return Result::Failure;
        }
    }
    return Result::Success;
...
SetSupplyForScenario(std::vector<ScheduleBasedSource>& loads, LoadDict loadMap, size_t scenarioIdx)
{
    for (size_t sblIdx = 0; sblIdx < loads.size(); ++sblIdx)
    {
        ...
        {
            std::cout << "ERROR:" << "Unhandled scenario id in ScenarioIdToSourceId" << std::endl;
            return Result::Failure;
        }
    }
    return Result::Success;
...
std::vector<double> DetermineScenarioOccurrenceTimes(Simulation& s, size_t scenIdx, bool isVerbose)
{
    ...
    if (isVerbose)
    {
        std::cout << "Occurrences: " << occurrenceTimes_s.size() << std::endl;
        for (size_t i = 0; i < occurrenceTimes_s.size(); ++i)
        {
            std::cout << "-- " << SecondsToPrettyString(occurrenceTimes_s[i]) << std::endl;
        }
    }
    return occurrenceTimes_s;
}

std::map<size_t, double> GetIntensitiesForScenario(Simulation& s, size_t scenIdx)
{
...
ApplyReliabilitiesAndFragilities(Simulation& s,
                                 double startTime_s,
                                 double endTime_s,
                                 ...
                                 std::map<size_t, std::vector<TimeState>> const& relSchByCompId)
{
    ...
    for (size_t cfmIdx = 0; cfmIdx < s.ComponentFailureModes.ComponentIds.size(); ++cfmIdx)
    {
        ...
        std::cout << "component: " << s.TheModel.ComponentMap.Tag[compId] << std::endl;
        std::cout << "initial age (h): " << (initialAge_s / seconds_per_hour) << std::endl;
        std::vector<TimeState> clip =
            TimeState_Clip(TimeState_Translate(sch, initialAge_s), startTime_s, endTime_s, true);
    ...
    if (intensityIdToAmount.size() > 0)
    {
        ...
        for (size_t cfmIdx = 0; cfmIdx < s.ComponentFragilities.ComponentIds.size(); ++cfmIdx)
        {
            size_t fmId = s.ComponentFragilities.FragilityModeIds[cfmIdx];
            size_t fcId = s.FragilityModes.FragilityCurveId[fmId];
            std::optional<size_t> repairId = s.FragilityModes.RepairDistIds[fmId];
            FragilityCurveType curveType = s.FragilityCurves.CurveTypes[fcId];
            ...
            case (FragilityCurveType::Linear):
            {
                LinearFragilityCurve lfc = s.LinearFragilityCurves[fcIdx];
            ...
            case (FragilityCurveType::Tabular):
            {
                TabularFragilityCurve tfc = s.TabularFragilityCurves[fcIdx];
            ...
            default:
            {
                WriteErrorMessage("fragility_curve", "unhandled fragility curve type");
                std::exit(1);
            }
            break;
            }
            ...
            if (isFailed)
            {
                ...
                // flag is true
                std::cout << "... FAILED: " << s.TheModel.ComponentMap.Tag[compId] << " (cause: "
                ...
                for (size_t rIdx = 0; rIdx < s.TheModel.Reliabilities.size(); ++rIdx)
                {
                    if (s.TheModel.Reliabilities[rIdx].ComponentId == compId)
                    {
                ...
                if (repairId.has_value())
                {
                    ...
                    double repairTime_s = s.TheModel.DistSys.next_time_advance(repId);
                ...
                {
                    auto const& currentSch = s.TheModel.Reliabilities[reliabilityId].TimeStates;
                    std::vector<TimeState> combined = TimeState_Combine(currentSch, newTimeStates);
                    s.TheModel.Reliabilities[reliabilityId].TimeStates = std::move(combined);
                }
                else
                {
                    ScheduleBasedReliability sbr {};
                           ...
                           std::vector<size_t> const& fragOrder)
{
    ...
    stats << "scenario id," << "occurrence number," << "duration (h)," << "total source (kJ),"
          << "total load (kJ)," << "total storage (kJ)," << "total waste (kJ),"
          << "energy balance (source-(load+storage+waste)) (kJ)," << "site efficiency,"
          << "uptime (h)," << "downtime (h)," << "load not served (kJ),"
          << "energy robustness [ER]," << "energy availability [EA],"
          << "max single event downtime [MaxSEDT] (h)," << "global availability";
    ...
    for (auto const& os : occurrenceStats)
    {
        ...
        double balance =
            os.Inflow_kJ + os.InFromEnv_kJ - (os.OutflowAchieved_kJ + stored + os.Wasteflow_kJ);
        double efficiency = (os.Inflow_kJ + os.StorageDischarge_kJ) > 0.0
                                ...
                                : 0.0;
        double ER =
            os.OutflowRequest_kJ > 0.0 ? (os.OutflowAchieved_kJ / os.OutflowRequest_kJ) : 0.0;
        double EA = os.Duration_s > 0.0 ? (os.Uptime_s / os.Duration_s) : 0.0;
...
void Simulation_Run(Simulation& s)
{
    ...
    for (size_t scenId = 0; scenId < s.ScenarioMap.Durations.size(); ++scenId)
    {
        double duration_s =
            Time_ToSeconds(s.ScenarioMap.Durations[scenId], s.ScenarioMap.TimeUnits[scenId]);
         ...
         ++compFailId)
    {
        ...
        double maxTime_s = Time_ToSeconds(s.Info.MaxTime, s.Info.TheTimeUnit) + maxDuration_s;
        std::vector<TimeState> relSch = s.TheModel.Rel.make_schedule_for_link(
    ...
    for (auto const& pair : relSchByCompFailId)
    {
        ...
        if (relSchByCompId.contains(compId))
        {
            std::vector<TimeState> combined =
                TimeState_Combine(pair.second, relSchByCompId.at(pair.first));
    ...
    if (!out.good())
    {
        std::cout << "Could not open '" << eventFilePath << "' for writing." << std::endl;
        return;
    }
    ...
    for (size_t scenIdx : scenarioOrder)
    {
        ...
        std::map<size_t, double> intensityIdToAmount = GetIntensitiesForScenario(s, scenIdx);
        for (size_t occIdx = 0; occIdx < occurrenceTimes_s.size(); ++occIdx)
        {
            ...
            std::cout << "Occurrence #" << (occIdx + 1) << " at " << SecondsToPrettyString(t)
                      << std::endl;
            // TODO: make initial age part of the ComponentMap
            std::vector<ScheduleBasedReliability> originalReliabilities =
                ApplyReliabilitiesAndFragilities(s, t, tEnd, intensityIdToAmount, relSchByCompId);
            std::string scenarioStartTimeTag =
                TimeToISO8601Period(static_cast<uint64_t>(std::llround(t)));
            if (option_verbose)
            {
                std::cout << "Running " << s.ScenarioMap.Tags[scenIdx] << " from "
                          << scenarioStartTimeTag << " for " << s.ScenarioMap.Durations[scenIdx]
                          << " " << TimeUnitToTag(s.ScenarioMap.TimeUnits[scenIdx]) << " ("
                          << SecondsToPrettyString(t) << " to " << SecondsToPrettyString(tEnd)
                          << ")" << std::endl;
            }
    ...
    out.close();
    WriteStatisticsToFile(s, statsFilePath, occurrenceStats, compOrder, failOrder, fragOrder);
}
} // namespace erin
