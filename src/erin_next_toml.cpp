                              ...
                              std::vector<std::string>& warnings)
{
    ...
    for (auto it = table.cbegin(); it != table.cend(); ++it)
    {
        ...
        else
        {
            ...
            {
                std::ostringstream oss {};
                oss << "WARNING! unhandled field '" << key << "' will be ignored";
                warnings.push_back(WriteErrorToString(tableName, oss.str()));
                continue;
            }
        }
    ...
    for (auto const& defkv : validationInfo.Defaults)
    {
        ...
        default:
        {
            WriteErrorMessage(tableName, "Parse error: unhandled datatype for default");
            std::exit(1);
        }
        }
                       ...
                       bool doPrint)
{
    for (auto it = table.cbegin(); it != table.cend(); ++it)
    {
            ...
            !defaults.contains(it->first))
        {
            if (doPrint)
            {
                std::cout << "[" << tableName << "] " << "Unrecognized key '" << it->first << "'"
                          << std::endl;
            }
            return false;
        }
    ...
    for (auto it = requiredFields.cbegin(); it != requiredFields.cend(); ++it)
    {
        if (!table.contains(*it))
        {
            if (doPrint)
            {
                std::cout << "[" << tableName << "] " << "Missing required key '" << *it << "'"
                          << std::endl;
            }
            return false;
        }
                                     ...
                                     double rateMult)
{
    ...
    {
        WriteErrorMessage(tableName, fieldName + " not present or not an array");
        return {};
    }
    ...
    for (size_t i = 0; i < trs.size(); ++i)
    {
        ...
        if (tr.is_array())
        {
            ...
            {
                WriteErrorMessage(tableName, "time/rate pair was not of length 2");
                return {};
            }
        }
        else
                                    ...
                                    std::string const& tableName)
{
    ...
    if (!table.contains(fieldName))
    {
        WriteErrorMessage(tableName, "does not contain required field '" + fieldName + "'");
        return {};
    }
    toml::value fieldData = table.at(fieldName);
    if (!fieldData.is_array())
    {
        WriteErrorMessage(tableName, fieldName + " must be an array of 2-element array of numbers");
        return {};
    }
