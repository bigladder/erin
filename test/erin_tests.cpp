...
TEST(ErinBasicsTest, Test_that_we_can_calculate_reliability_schedule)
{
  ...
  std::vector<int64_t> fixed_distribution_attr_value{5, 1};
  ...
  std::vector<int64_t> comp_id_to_time(num_components, 0);
  std::unordered_map<size_type, std::vector<int64_t>> comp_id_to_reliability_schedule_times{};
  ...
  for (size_type component_id{0}; component_id < num_components; ++component_id) {
    comp_id_to_reliability_schedule_times.emplace(component_id, std::vector<int64_t>{0});
  ...
  //std::unordered_map<size_type, std::vector<int64_t>> comp_id_to_reliability_schedule_times{};
  //std::unordered_map<size_type, std::vector<bool>> comp_id_to_reliability_schedule_states{};
  EXPECT_EQ(comp_id_to_reliability_schedule_times.size(), 1);
  std::vector<int64_t> expected_times{0, 5, 6, 11};
  EXPECT_EQ(comp_id_to_reliability_schedule_times.at(0), expected_times);
  std::vector<bool> expected_states{true, false, true, false};
  EXPECT_EQ(comp_id_to_reliability_schedule_states.at(0), expected_states);
}
