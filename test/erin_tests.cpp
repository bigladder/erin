...
static double Round(double n, unsigned int places = 2) {
...
TEST(Erin, Test1) {
    ...
    EXPECT_TRUE(srcToLoadResult.has_value()) << "connection result should have a value";
    EXPECT_EQ(srcToLoadResult.value().Actual_W, 10) << "actual value must equal 10";
    EXPECT_EQ(srcToLoadResult.value().Available_W, 100) << "available must equal 100";
    EXPECT_EQ(srcToLoadResult.value().Requested_W, 10) << "requested must equal 10";
}
...
TEST(Erin, Test2) {
    ...
    EXPECT_TRUE(srcToConvResults.has_value()) << "source to converter must have results";
    EXPECT_EQ(srcToConvResults.value().Requested_W, 20) << "requested must equal 20";
    EXPECT_EQ(srcToConvResults.value().Actual_W, 20) << "actual value must equal 20";
    EXPECT_EQ(srcToConvResults.value().Available_W, 100) << "available must equal 100";

    auto convToLoadResults = ModelResults_GetFlowForConnection(m, convToLoadConn, 0.0, results);
    EXPECT_TRUE(convToLoadResults.has_value()) << "converter to load must have results";
    EXPECT_EQ(convToLoadResults.value().Requested_W, 10) << "requested must equal 10";
    EXPECT_EQ(convToLoadResults.value().Actual_W, 10) << "actual value must equal 10";
    EXPECT_EQ(convToLoadResults.value().Available_W, 50) << "available must equal 50";

    auto convToWasteResults =
            ModelResults_GetFlowForConnection(m, convId.WasteConnection, 0.0, results);
    EXPECT_TRUE(convToWasteResults.has_value()) << "converter to waste must have results";
    EXPECT_EQ(convToWasteResults.value().Requested_W, 10) << "requested must equal 10";
    EXPECT_EQ(convToWasteResults.value().Actual_W, 10) << "actual value must equal 10";
    EXPECT_EQ(convToWasteResults.value().Available_W, 10) << "available must equal 10";
}
...
TEST(Erin, Test3) {
    ...
    EXPECT_TRUE(convToLoad1Results.has_value()) << "converter to load1 must have results";
    EXPECT_EQ(convToLoad1Results.value().Requested_W, 10) << "requested must equal 10";
    EXPECT_EQ(convToLoad1Results.value().Actual_W, 10) << "actual value must equal 10";
    EXPECT_EQ(convToLoad1Results.value().Available_W, 50) << "available must equal 50";

    auto convToLoad2Results = ModelResults_GetFlowForConnection(m, convToLoad2Conn, 0.0, results);
    EXPECT_TRUE(convToLoad2Results.has_value()) << "conv to load2 must have results";
    EXPECT_EQ(convToLoad2Results.value().Requested_W, 2) << "requested must equal 2";
    EXPECT_EQ(convToLoad2Results.value().Actual_W, 2) << "actual value must equal 2";
    EXPECT_EQ(convToLoad2Results.value().Available_W, 10) << "available must equal 10";

    auto convToWasteResults =
            ModelResults_GetFlowForConnection(m, convId.WasteConnection, 0.0, results);
    EXPECT_TRUE(convToWasteResults.has_value()) << "conv to waste must have results";
    EXPECT_EQ(convToWasteResults.value().Requested_W, 8) << "requested must equal 8";
    EXPECT_EQ(convToWasteResults.value().Actual_W, 8) << "actual value must equal 8";
    EXPECT_EQ(convToWasteResults.value().Available_W, 8) << "available must equal 8";
}
...
TEST(Erin, Test4) {
    ...
    EXPECT_TRUE(srcToLoadResults_0.has_value()) << "source to load must have results at time=0.0";
    EXPECT_EQ(srcToLoadResults_0.value().Requested_W, 10) << "requested must equal 10";
    EXPECT_EQ(srcToLoadResults_0.value().Actual_W, 10) << "actual value must equal 10";
    EXPECT_EQ(srcToLoadResults_0.value().Available_W, 100) << "available must equal 100";
    ...
    EXPECT_EQ(srcToLoadResults_3600.value().Requested_W, 200) << "requested must equal 200";
    EXPECT_EQ(srcToLoadResults_3600.value().Actual_W, 100) << "actual value must equal 100";
    EXPECT_EQ(srcToLoadResults_3600.value().Available_W, 100) << "available must equal 100";
}
...
TEST(Erin, Test5) {
    ...
    EXPECT_EQ(srcToConv1Results.value().Actual_W, 40) << "src to conv1 should flow 40";
    EXPECT_EQ(conv1ToLoad1Results.value().Actual_W, 10) << "conv1 to load1 should flow 10";
    EXPECT_EQ(conv1ToConv2Results.value().Actual_W, 28) << "conv1 to conv2 should flow 28";
    EXPECT_EQ(conv2ToLoad2Results.value().Actual_W, 7) << "conv1 to conv2 should flow 7";
    EXPECT_EQ(conv2ToConv3Results.value().Actual_W, 20) << "conv2 to conv3 should flow 21";
    EXPECT_EQ(conv3ToLoad3Results.value().Actual_W, 5) << "conv3 to load3 should flow 5";
}
...
TEST(Erin, Test6) {
    ...
    EXPECT_EQ(src1ToMuxResults.value().Actual_W, 10) << "src1 -> mux expected actual flow of 10";

    auto src2ToMuxResults = ModelResults_GetFlowForConnection(m, src2ToMuxConn, 0.0, results);
    EXPECT_EQ(src2ToMuxResults.value().Actual_W, 50) << "src2 -> mux expected actual flow of 50";

    auto muxToLoad1Results = ModelResults_GetFlowForConnection(m, muxToLoad1Conn, 0.0, results);
    EXPECT_EQ(muxToLoad1Results.value().Actual_W, 10) << "mux -> load1 expected actual flow of 10";

    auto muxToLoad2Results = ModelResults_GetFlowForConnection(m, muxToLoad2Conn, 0.0, results);
    EXPECT_EQ(muxToLoad2Results.value().Actual_W, 50) << "mux -> load2 expected actual flow of 50";
}
...
TEST(Erin, Test7) {
    ...
    EXPECT_EQ(srcToStoreResults.value().Actual_W, 0) << "src to store should be providing 0";
    EXPECT_EQ(srcToStoreResults.value().Requested_W, 10) << "src to store request is 10";
    EXPECT_EQ(srcToStoreResults.value().Available_W, 0) << "src to store available is 0";
    ...
    EXPECT_EQ(storeToLoadResults.value().Actual_W, 10) << "store to load should be providing 10";
    EXPECT_EQ(storeToLoadResults.value().Requested_W, 10)
                        << "store to load should be requesting 10";
    EXPECT_EQ(storeToLoadResults.value().Available_W, 10) << "store to load available should be 10";
    ...
    EXPECT_EQ(srcToStoreResultsAt10.value().Actual_W, 0) << "src to store should be providing 0";
    EXPECT_EQ(srcToStoreResultsAt10.value().Requested_W, 20) << "src to store request is 20";
    EXPECT_EQ(srcToStoreResultsAt10.value().Available_W, 0) << "src to store available is 0";
    ...
    EXPECT_TRUE(storeToLoadResultsAt10.has_value())
                        << "should have results for store to load connection";
    EXPECT_EQ(storeToLoadResultsAt10.value().Actual_W, 0) << "store to load should be providing 0";
...
TEST(Erin, Test8) {
    ...
    auto results = Simulate(m, false);
    auto srcToStoreResults = ModelResults_GetFlowForConnection(m, srcToStoreConn, 0.0, results);
    EXPECT_EQ(srcToStoreResults.value().Actual_W, 5) << "src to store should be providing 5";
    EXPECT_EQ(srcToStoreResults.value().Requested_W, 10) << "src to store request is 10";
    EXPECT_EQ(srcToStoreResults.value().Available_W, 5) << "src to store available is 5";
    ...
    EXPECT_EQ(storeToLoadResults.value().Requested_W, 10)
                        << "store to load should be requesting 10";
    EXPECT_EQ(storeToLoadResults.value().Available_W, 15) << "store to load available should be 15";
    EXPECT_EQ(results.size(), 2) << "there should be two time events in results";
    ...
    EXPECT_EQ(srcToStoreResultsAt20.value().Actual_W, 5) << "src to store should be providing 5";
    EXPECT_EQ(srcToStoreResultsAt20.value().Requested_W, 20) << "src to store request is 20";
    EXPECT_EQ(srcToStoreResultsAt20.value().Available_W, 5) << "src to store available is 5";
    ...
    EXPECT_TRUE(storeToLoadResultsAt20.has_value())
                        << "should have results for store to load connection";
    EXPECT_EQ(storeToLoadResultsAt20.value().Actual_W, 5) << "store to load should be providing 5";
...
TEST(Erin, Test10) {
    ...
    auto src1ToMuxResults = ModelResults_GetFlowForConnection(m, src1ToMux0Port0Conn, 0.0, results);
    ...
    auto src2ToStoreResults = ModelResults_GetFlowForConnection(m, src2ToStoreConn, 0.0, results);
    ...
    auto storeToMuxResults =
            ModelResults_GetFlowForConnection(m, storeToMux0Port1Conn, 0.0, results);
    ...
    auto muxToLoad1Results =
            ModelResults_GetFlowForConnection(m, mux0Port0ToLoad1Conn, 0.0, results);
    ...
    auto muxToConvResults = ModelResults_GetFlowForConnection(m, mux0Port1ToConvConn, 0.0, results);
    ...
    auto convToLoad3Results = ModelResults_GetFlowForConnection(m, convToLoad3Conn, 0.0, results);
    ...
    convToWasteResults = ModelResults_GetFlowForConnection(m, conv.WasteConnection, t, results);
    EXPECT_EQ(convToWasteResults.value().Actual_W, 3);
    EXPECT_EQ(convToWasteResults.value().Requested_W, 3);
    EXPECT_EQ(convToWasteResults.value().Available_W, 3);

    src1ToMuxResults = ModelResults_GetFlowForConnection(m, src1ToMux0Port0Conn, t, results);
    EXPECT_EQ(src1ToMuxResults.value().Actual_W, 20);
    EXPECT_EQ(src1ToMuxResults.value().Available_W, 20);
    EXPECT_EQ(src1ToMuxResults.value().Requested_W, 60);

    src2ToStoreResults = ModelResults_GetFlowForConnection(m, src2ToStoreConn, t, results);
    EXPECT_EQ(src2ToStoreResults.value().Actual_W, 5);
    EXPECT_EQ(src2ToStoreResults.value().Available_W, 5);
    EXPECT_EQ(src2ToStoreResults.value().Requested_W, 50);

    storeToMuxResults = ModelResults_GetFlowForConnection(m, storeToMux0Port1Conn, t, results);
    EXPECT_EQ(storeToMuxResults.value().Actual_W, 15);
    EXPECT_EQ(storeToMuxResults.value().Available_W, 15);
    EXPECT_EQ(storeToMuxResults.value().Requested_W, 40);

    muxToLoad1Results = ModelResults_GetFlowForConnection(m, mux0Port0ToLoad1Conn, t, results);
    EXPECT_EQ(muxToLoad1Results.value().Actual_W, 20);
    EXPECT_EQ(muxToLoad1Results.value().Available_W, 20);
    EXPECT_EQ(muxToLoad1Results.value().Requested_W, 20);

    muxToConvResults = ModelResults_GetFlowForConnection(m, mux0Port1ToConvConn, t, results);
    EXPECT_EQ(muxToConvResults.value().Actual_W, 15);
    EXPECT_EQ(muxToConvResults.value().Available_W, 15);
    EXPECT_EQ(muxToConvResults.value().Requested_W, 40);

    convToLoad2Results = ModelResults_GetFlowForConnection(m, convToLoad2Conn, t, results);
    EXPECT_EQ(convToLoad2Results.value().Actual_W, 7);
    EXPECT_EQ(convToLoad2Results.value().Available_W, 7);
    EXPECT_EQ(convToLoad2Results.value().Requested_W, 20);

    convToLoad3Results = ModelResults_GetFlowForConnection(m, convToLoad3Conn, t, results);
    EXPECT_EQ(convToLoad3Results.value().Actual_W, 5);
    EXPECT_EQ(convToLoad3Results.value().Available_W, 8);
    EXPECT_EQ(convToLoad3Results.value().Requested_W, 5);
...
TEST(Erin, Test11) {
    ...
    double t = 0.0;
    auto srcToConvResults = ModelResults_GetFlowForConnection(m, srcToConvConn, t, results);
    EXPECT_EQ(srcToConvResults.value().Actual_W, 20) << "src -> conv actual should be 20";
    EXPECT_EQ(srcToConvResults.value().Requested_W, 20) << "src -> conv requested should be 20";
    EXPECT_EQ(srcToConvResults.value().Available_W, 100) << "src -> conv available should be 100";

    auto convToLoadResults = ModelResults_GetFlowForConnection(m, convToLoadConn, t, results);
    EXPECT_EQ(convToLoadResults.value().Actual_W, 10) << "conv -> load actual should be 10";
    EXPECT_EQ(convToLoadResults.value().Requested_W, 10) << "conv -> load requested should be 10";
    EXPECT_EQ(convToLoadResults.value().Available_W, 50) << "conv -> load available should be 50";

    auto convToWasteResults =
            ModelResults_GetFlowForConnection(m, convId.WasteConnection, t, results);
    EXPECT_EQ(convToWasteResults.value().Actual_W, 10) << "conv -> waste actual should be 10";
    EXPECT_EQ(convToWasteResults.value().Requested_W, 10) << "conv -> waste requested should be 10";
    EXPECT_EQ(convToWasteResults.value().Available_W, 10) << "conv -> waste available should be 10";

    // time = 10.0, failed
    t = 10.0;
    srcToConvResults = ModelResults_GetFlowForConnection(m, srcToConvConn, t, results);
    EXPECT_EQ(srcToConvResults.value().Actual_W, 0) << "src -> conv actual should be 0";
    EXPECT_EQ(srcToConvResults.value().Requested_W, 0) << "src -> conv requested should be 0";
    EXPECT_EQ(srcToConvResults.value().Available_W, 100) << "src -> conv available should be 100";

    convToLoadResults = ModelResults_GetFlowForConnection(m, convToLoadConn, t, results);
    EXPECT_EQ(convToLoadResults.value().Actual_W, 0) << "conv -> load actual should be 0";
    EXPECT_EQ(convToLoadResults.value().Requested_W, 10) << "conv -> load requested should be 10";
    EXPECT_EQ(convToLoadResults.value().Available_W, 0) << "conv -> load available should be 0";

    convToWasteResults = ModelResults_GetFlowForConnection(m, convId.WasteConnection, t, results);
    EXPECT_EQ(convToWasteResults.value().Actual_W, 0) << "conv -> waste actual should be 0";
    EXPECT_EQ(convToWasteResults.value().Requested_W, 0) << "conv -> waste requested should be 0";
    EXPECT_EQ(convToWasteResults.value().Available_W, 0) << "conv -> waste available should be 0";
    ...
    EXPECT_EQ(convToLoadResults.value().Actual_W, 10) << "conv -> load actual should be 10";
    EXPECT_EQ(convToLoadResults.value().Requested_W, 10) << "conv -> load requested should be 10";
    EXPECT_EQ(convToLoadResults.value().Available_W, 50) << "conv -> load available should be 0";
...
TEST(Erin, Test12) {
    ...
    double t = 0.0;
    auto srcToLoadResults = ModelResults_GetFlowForConnection(m, srcToLoadConn, t, results);
    ...
    t = 10.0;
    srcToLoadResults = ModelResults_GetFlowForConnection(m, srcToLoadConn, t, results);
    EXPECT_EQ(srcToLoadResults.value().Actual_W, 8);
    EXPECT_EQ(srcToLoadResults.value().Available_W, 8);
    EXPECT_EQ(srcToLoadResults.value().Requested_W, 10);
    srcToWasteResults = ModelResults_GetFlowForConnection(m, srcId.WasteConnection, t, results);
    EXPECT_EQ(srcToWasteResults.value().Actual_W, 0);
    EXPECT_EQ(srcToWasteResults.value().Available_W, 0);
    EXPECT_EQ(srcToWasteResults.value().Requested_W, 0);
    t = 20.0;
...
TEST(Erin, Test13) {
    ...
    heatLoad.reserve(49);
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(0.0), kW_as_W(29.60017807)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(1.0), kW_as_W(16.70505099)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(2.0), kW_as_W(16.99812206)});
    ...
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(5.0), kW_as_W(23.73875667)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(6.0), kW_as_W(17.87733527)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(7.0), kW_as_W(24.03182774)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(8.0), kW_as_W(17.87733527)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(9.0), kW_as_W(23.4456856)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(10.0), kW_as_W(16.41197992)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(11.0), kW_as_W(18.75654848)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(12.0), kW_as_W(14.36048243)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(13.0), kW_as_W(16.11890885)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(14.0), kW_as_W(10.55055852)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(15.0), kW_as_W(13.77434029)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(16.0), kW_as_W(9.37827424)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(17.0), kW_as_W(13.18819815)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(18.0), kW_as_W(9.37827424)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(19.0), kW_as_W(13.48126922)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(20.0), kW_as_W(9.67134531)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(21.0), kW_as_W(12.30898494)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(22.0), kW_as_W(10.55055852)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(23.0), kW_as_W(13.48126922)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(24.0), kW_as_W(9.67134531)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(25.0), kW_as_W(13.48126922)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(26.0), kW_as_W(12.30898494)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(27.0), kW_as_W(14.06741136)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(28.0), kW_as_W(12.30898494)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(29.0), kW_as_W(13.48126922)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(30.0), kW_as_W(10.84362959)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(31.0), kW_as_W(4.10299498)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(32.0), kW_as_W(45.71908692)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(33.0), kW_as_W(38.97845231)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(34.0), kW_as_W(33.11703091)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(35.0), kW_as_W(26.96253844)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(36.0), kW_as_W(24.32489881)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(37.0), kW_as_W(22.85954346)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(38.0), kW_as_W(26.66946737)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(39.0), kW_as_W(29.89324914)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(40.0), kW_as_W(26.66946737)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(41.0), kW_as_W(24.32489881)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(42.0), kW_as_W(27.25560951)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(43.0), kW_as_W(26.66946737)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(44.0), kW_as_W(22.85954346)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(45.0), kW_as_W(21.10111704)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(46.0), kW_as_W(18.46347741)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(47.0), kW_as_W(0.0)});
    heatLoad.push_back(TimeAndAmount{hours_as_seconds(48.0), kW_as_W(3.22378177)});
    ...
    // NETWORK / CONNECTIONS
    // - electricity
    auto pvToEmuxConn = Model_AddConnection(m, pvArrayId.Id, 0, elecSourceMuxId, 0);
    auto eutilToEmuxConn = Model_AddConnection(m, elecUtilId, 0, elecSourceMuxId, 1);
    auto emuxToBatteryConn = Model_AddConnection(m, elecSourceMuxId, 0, batteryId, 0);
    auto batteryToEsupplyConn = Model_AddConnection(m, batteryId, 0, elecSupplyMuxId, 0);
    auto ngGenToEsupplyConn = Model_AddConnection(m, ngToElecConvId.Id, 0, elecSupplyMuxId, 1);
    auto esupplyToLoadConn = Model_AddConnection(m, elecSupplyMuxId, 0, elecLoadId, 0);
    auto esupplyToHeatPumpConn =
            Model_AddConnection(m, elecSupplyMuxId, 1, elecHeatPumpConvId.Id, 0);
    // - natural gas
    auto ngGridToNgMuxConn = Model_AddConnection(m, ngUtilId, 0, ngSourceMuxId, 0);
    auto ngMuxToNgGenConn = Model_AddConnection(m, ngSourceMuxId, 0, ngToElecConvId.Id, 0);
    auto ngMuxToNgHeaterConn = Model_AddConnection(m, ngSourceMuxId, 1, ngHeaterConvId.Id, 0);
    // - heating
    ...
    auto heatMuxToLoadConn = Model_AddConnection(m, heatingSupplyMuxId, 0, heatLoadId, 0);
    // SIMULATE
    auto results = Simulate(m, false);
}
...
TEST(Erin, Test15) {
    ...
    double t = 0.0;
    auto src1ToConvResults = ModelResults_GetFlowForConnection(m, src1ToConvConn, t, results);
    ...
    auto convLossToMuxResults = ModelResults_GetFlowForConnection(m, convLossToMuxConn, t, results);
    ...
    auto src2ToMuxResults = ModelResults_GetFlowForConnection(m, src2ToMuxConn, t, results);
    ...
    auto muxToLoadResults = ModelResults_GetFlowForConnection(m, muxToLoadConn, t, results);
    ...
    t = 2.0;
    src1ToConvResults = ModelResults_GetFlowForConnection(m, src1ToConvConn, t, results);
    EXPECT_TRUE(src1ToConvResults.has_value());
    ...
    convLossToMuxResults = ModelResults_GetFlowForConnection(m, convLossToMuxConn, t, results);
    EXPECT_TRUE(convLossToMuxResults.has_value());
    EXPECT_EQ(convLossToMuxResults.value().Actual_W, 30);
    EXPECT_EQ(convLossToMuxResults.value().Requested_W, 100);
    EXPECT_EQ(convLossToMuxResults.value().Available_W, 30);

    muxToLoadResults = ModelResults_GetFlowForConnection(m, muxToLoadConn, t, results);
    EXPECT_TRUE(muxToLoadResults.has_value());
    EXPECT_EQ(muxToLoadResults.value().Actual_W, 100);
    EXPECT_EQ(muxToLoadResults.value().Requested_W, 100);
    EXPECT_EQ(muxToLoadResults.value().Available_W, 1'030);

    src2ToMuxResults = ModelResults_GetFlowForConnection(m, src2ToMuxConn, t, results);
    EXPECT_TRUE(src2ToMuxResults.has_value());
...
TEST(Erin, Test16) {
    ...
    double t = 0.0;
    auto srcToPassResults = ModelResults_GetFlowForConnection(m, srcToPassConn, t, results);
    ...
    auto passToLoadResults = ModelResults_GetFlowForConnection(m, passToLoadConn, t, results);
...
TEST(Erin, Test17) {
    std::vector<TimeState> a{{0.0,   true},
                             {10.0,  false, {1}},
                             {100.0, true}};
    std::vector<TimeState> b{{0.0,   true},
                             {40.0,  false, {2}},
                             {90.0,  true},
                             {150.0, false, {2}}};
...
TEST(Erin, Test21) {
    ...
    TimeState_CountAndTimeFailureEvents(
            input, 200.0, countByFailModeId, countByFragModeId, timeByFailModeId, timeByFragModeId);
    ...
    TimeState_CountAndTimeFailureEvents(
            input, 180.0, countByFailModeId, countByFragModeId, timeByFailModeId, timeByFragModeId);
    EXPECT_EQ(expectedCountByFailModeId.size(), countByFailModeId.size());
    EXPECT_EQ(expectedCountByFragModeId.size(), countByFragModeId.size());
    EXPECT_EQ(expectedTimeByFailModeId.size(), timeByFailModeId.size());
    EXPECT_EQ(expectedTimeByFragModeId.size(), timeByFragModeId.size());
