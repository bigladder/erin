...
TEST(Erin, TestParsingComponentsInUse)
{
    ...
    std::unordered_set<std::string> actual = erin::TOMLTable_ParseComponentTagsInUse(exampleInput);
    EXPECT_EQ(expected.size(), actual.size());
...
TEST(Erin, TestApplyUniformTimeStep)
{
    ...
    // NETWORK / CONNECTIONS
    Model_AddConnection(m, ePV.Id, 0, eBattId, 0);
    Model_AddConnection(m, eBattId, 0, eLoadId, 0);
